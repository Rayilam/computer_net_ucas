...
void do_translation(iface_info_t *iface, char *packet, int len, int dir)
{
	...
	u32 rm_ip = dir == DIR_IN ? ntohl(iph->saddr) : ntohl(iph->daddr);
	u16 rm_port = dir == DIR_IN ? ntohs(tcph->sport) : ntohs(tcph->dport);

	char tmp[6];
	memcpy(tmp, (char *)&rm_ip, 4);
	memcpy(tmp + 4, (char *)&rm_port, 2);
...
int parse_config(const char *filename)
{
	...
	while (fgets(line, 100, fd) != NULL)
	{
		...
		else if (strstr(line, "dnat-rules"))
		{
			...
			u32 external_ip = ip_tr(ip1);
			u32 internal_ip = ip_tr(ip2);
